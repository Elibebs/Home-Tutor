...
class VerificationRepo extends BaseRepo
{
	public function createVerificationEntry(Array $data)
	{
		...
		$existingVerification = Verification::where("phone_number", $data['phone_number'])->where("is_active", true)->first();
	...
	public function attemptVerifyPin(String $pin, $phone_number)
	{
		...
		$verification = Verification::where("phone_number", $phone_number)->where("is_active", true)->first();
